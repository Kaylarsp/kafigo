...
interface Transaction {
  ...
  account?: Account | null; // tambahkan ini untuk relasi account
...
export default function Transactions({ transactions = [], accounts = [] }: Props) {
  // Convert amounts to numbers when needed
  ...
  const formatFullCurrency = (val: number) =>
    `IDR ${Number(val).toLocaleString('id-ID', { minimumFractionDigits: 2 })}`;
  ...
  function openEdit(tx: Transaction) {
    setData('id', tx.id);
    setData('account_id', (tx.account_id ?? '').toString());
    setData('amount', Number(tx.amount || 0).toString());
    setData('type', tx.type);
    setData('description', tx.description ?? '');
    setData('transaction_date', tx.transaction_date ?? new Date().toISOString().slice(0, 10));
    setData('title', tx.title ?? '');
    setIsFormOpen(true);
  }
      ...
      <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 dark:from-slate-900 dark:to-slate-800">
        <div className="flex flex-col gap-6 p-6">
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-slate-600 dark:text-slate-400 mt-1">
                Track all your incomes, expenses, and transfers here.
              </p>
            </div>
          ...
          <div className="grid gap-6 md:grid-cols-3">
            ...
            {/* Outcome */}
            <Card className="relative overflow-hidden border-0 shadow-lg bg-gradient-to-br from-red-600 to-red-700 text-white">
              <div className="absolute top-0 right-0 w-20 h-20 bg-white/10 rounded-full -translate-y-10 translate-x-10" />
              <CardHeader className="pb-2 relative z-10">
                <div className="flex items-center justify-between">
              ...
              <CardContent className="relative z-10">
                <div className="text-3xl font-bold mb-2">{formatCurrency(totalOutcome)}</div>
                <div className="flex items-center text-sm opacity-90">
            ...
            <Card className="relative overflow-hidden border-0 shadow-lg bg-gradient-to-br from-blue-600 to-blue-700 text-white">
              ...
              <CardHeader className="pb-2 relative z-10">
                <div className="flex items-center justify-between">
                  <CardTitle className="text-sm font-medium opacity-90">Transfer</CardTitle>
                  <ArrowRightLeft className="w-5 h-5 opacity-80" />
                </div>
              </CardHeader>
              <CardContent className="relative z-10">
                <div className="text-3xl font-bold mb-2">{formatCurrency(totalTransfer)}</div>
                <div className="flex items-center text-sm opacity-90">
                  <span>{formatFullCurrency(totalTransfer)} total</span>
                </div>
              </CardContent>
            </Card>
          </div>
      ...
      <Dialog open={isViewOpen} onOpenChange={setIsViewOpen}>
        <DialogContent>
          ...
          <div className="space-y-2 mt-2">
            ...
            <div className="text-sm">
              <strong>Amount:</strong> {selected ? formatFullCurrency(Number(selected.amount)) : '-'}
            </div>
            <div className="text-sm">
              <strong>Date:</strong> {selected?.transaction_date ?? '-'}
            </div>
            <div className="text-sm">
              <strong>Account ID:</strong> {selected?.account_id ?? '-'}
            </div>
            <div className="text-sm">
          ...
          <DialogFooter className="mt-4">
            <div className="flex gap-2">
              <Button onClick={() => { if (selected) openEdit(selected); }}>Edit</Button>
        ...
        <DialogContent>
          ...
          <form onSubmit={submit} className="space-y-3 mt-2">
            ...
            <div>
              ...
              <select
                id="account_id"
                className="w-full rounded-md border px-3 py-2" // Menggunakan style yang sama dengan select 'Type' Anda
              ...
              {errors.account_id && <div className="text-xs text-red-600">{errors.account_id}</div>}
            </div>
            ...
            <div>
              ...
              {errors.transaction_date && <div className="text-xs text-red-600">{errors.transaction_date}</div>}
            </div>
            ...
            <DialogFooter className="mt-2">
              <div className="flex gap-2">
                ...
                <Button variant="ghost" onClick={() => { reset(); setIsFormOpen(false); }}>
                  Cancel
                </Button>
              </div>
            </DialogFooter>
